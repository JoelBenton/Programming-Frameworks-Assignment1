...
const Flashcard: React.FC<FlashcardProps> = ({ flashcard, user, onSubmit }) => {
    const handlePreview = () => {
        onSubmit(flashcard);
    };
    return (
            ...
            <Card className="bg-gradient-to-br from-[#c2e0d3] to-[#b3c7f9] rounded-xl shadow-xl p-4 transition-transform transform hover:scale-105 relative">
                <CardContent className="flex flex-col items-center">
                    ...
                    <div className="p-2 pt-4">
                        <p className="text-gray-500 mt-auto absolute bottom-4 left-4">
                            {user?.username}
                        </p>
                        <button
                            onClick={handlePreview}
                            className="absolute bottom-4 right-4 bg-[#b3c7f9] border-gray-600 border-2 text-white px-4 rounded hover:bg-[#8aabfe] transition h-8"
                        >
                            Preview
                        </button>
                    </div>
