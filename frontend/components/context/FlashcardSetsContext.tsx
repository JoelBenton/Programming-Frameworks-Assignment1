...
export const FlashcardSetsProvider: React.FC<FlashcardSetsProviderProps> = ({
    children,
}) => {
    // Local state to hold flashcard sets
    const [flashcardSets, setFlashcardSets] = useState<FlashcardSet[] | null>(
        null,
    );

    // Function to load flashcards based on API type
    const loadFlashcards = useCallback(
        async (apiType: "user" | "all", userId?: string) => {
            try {
    ...
    return (
        <FlashcardSetsContext.Provider
            value={{ flashcardSets, loadFlashcards }}
        >
            {children}
        </FlashcardSetsContext.Provider>
    );
