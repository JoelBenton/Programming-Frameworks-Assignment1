...
export const FlashcardCommmentSetProvider: React.FC<FlashcardSetsProviderProps> = ({ children }) => {
  // Local state to hold flashcard sets
  const [flashcardCommentSet, setFlashcardCommentSet] = useState<FlashcardCommentSet | null>(null);

  // Function to load flashcards based on API type
  const loadFlashcardCommentSet = useCallback(async (flashcardSetId: string) => {
    try {
  ...
  return (
    <FlashcardCommentSetContext.Provider value={{ flashcardCommentSet, loadFlashcardCommentSet }}>
      {children}
