...
export default function UserManagementPage() {
  ...
  const { users, refreshUsers } = useUsers() as { users: User[] | null; refreshUsers: () => void };
  ...
  useEffect(() => {
    async function loadLimits() {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_BASE}/api/sets/get-limit-info`, {
          method: "GET",
          headers: {
        ...
        if (response.ok) {
          const data = await response.json() as { limit: number, totalRequests: number}
          setLimit(data.limit)
  ...
  const handleUserUpdateSubmit = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_BASE}/api/users/${selectedUser.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleLimitSubmit = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL_BASE}/api/sets/update-limit`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${session?.user.token}`,
    ...
    <div className="flex flex-col items-center space-y-6 mb-20">
      ...
      <div className="flex flex-col items-center w-[50%] border-black border-2 p-4 rounded-xl bg-gradient-to-br from-[#1a1a3d] to-[#5dbdbd]">
        <div className="flex flex-col items-center space-y-2">
          <label htmlFor="user-select" className="text-xl font-medium">
            Select a User
          </label>
          <select
            id="user-select"
      ...
      <div className="flex flex-col items-center w-[50%] border-black border-2 p-6 rounded-xl bg-gradient-to-br from-[#1a1a3d] to-[#5dbdbd] mt-8 shadow-lg">
        ...
        <div className="w-full text-center text-white space-y-4">
          ...
          <div className="flex justify-between items-center border-b border-gray-300 pb-2">
            <span className="text-lg font-medium">Total Requests:</span>
            <span className="text-xl font-semibold">{totalRequests}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-lg font-medium">Remaining Requests:</span>
            <span className="text-xl font-semibold">{remainingRequests}</span>
          </div>
        </div>
