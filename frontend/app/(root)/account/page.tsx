...
const Page = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!currentPassword) {
      setError('Current Password required for either Changing Username or password')
      return
    }
    ...
    if (newUsername && newPassword) {
      ...
      credentials = { username: session.user.username, password: newPassword }
    } else {
      setError('No changes detected. Please provide a new username or password.')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(errorData.message || 'An error occurred while updating the user details.')
        return
      }
  ...
  return (
    <div className="bg-gradient-to-br from-[#09092c] to-[#634698] text-white min-h-screen flex flex-col items-center justify-center p-8">
      <div className="text-3xl font-bold mb-6">{session?.user.username}</div>
      ...
      <div className="w-full max-w-lg bg-[#1e1e2f] p-6 rounded-2xl shadow-lg">
        ...
        <div className="text-2xl font-semibold mb-4">Current Password <span className="text-red-500">*</span></div>
        <input
        ...
        <div className="text-2xl font-semibold mb-4">Change Username</div>
        <input
        ...
        <div className="text-2xl font-semibold mb-4">Change Password</div>
        <input
